...
- [x] 82.2 **右侧面板生效规则两段式折叠抽屉设计 (`web/index.html`, `web/style.css`)**：
  - 在「生效规则定义」操作行「+ 新增规则」前增加「AI智能生成」文本按钮（附带 Lucide Sparkles SVG 矢量图标，杜绝 emoji）；
  - 点击平滑展开嵌入式抽屉卡片，包含场景需求多行输入框、在线大模型下拉选择器及「智能生成」按钮；
  - 遵循现代极简中性灰设计风格（`.ai-gen-drawer`, `.ai-gen-textarea`, `.ai-gen-result-box`），不引入任何 emoji 图标。
- [x] 82.3 **候选字段审查与原生另存模板弹窗联动 (`web/app.js`)**：
  - AI 生成结果实时渲染为多选候选列表（包含字段名、风险优先级徽标与提取特征描述）；
  - 点击「仅应用到当前规则」直接注入当前生效规则并收起抽屉；
  - 点击「保存为模板并生效」先更新生效规则，再自动呼出既有的原生「另存为场景模板」模态弹窗（`#saveTemplateModal`），保持操作体验一致。
- [x] 82.4 **端到端实机验证与质量闭环**：
  - 25 项 Rust 单元测试（`cargo test`）全绿通过；
  - Chrome 实机端到端模拟测试（需求输入 -> 模型请求 -> 字段渲染 -> 勾选应用 -> 模态弹窗弹出）全部正常。
