...
- [x] 82.2 **右侧面板操作按钮统合、标题极简化与轻质抽屉面板重构 (`web/index.html`, `web/style.css`)**：
  ...
  - 将「AI智能生成」精简为「**AI生成**」，将「常用标签库」升级为轻质折叠抽屉并前置「**标签库**」实体按钮；
  - 将「**另存为模板**」由文字链接重构为与「新增规则」同款的标准实体按钮样式（`.btn.sm`），实现右侧面板操作按钮体系的全量质感对齐。
...
- [x] 82.4 **端到端实机验证与质量闭环**：
  - 25 项 Rust 单元测试（`cargo test`）全绿通过；
  - Chrome 实机端到端模拟测试（全按钮样式像素级统一、另存模板弹窗拉起、抽屉柔和卡片质感、标题纯粹无尾缀、标签库与AI生成互斥联动）全部正常。
