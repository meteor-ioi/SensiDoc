...
- [x] 82.2 **右侧面板操作按钮统合与两段式折叠抽屉设计 (`web/index.html`, `web/style.css`)**：
  - 将「AI智能生成」与「新增规则」按钮全面统合为与「导入文件」同款的实体按钮样式（`.btn.sm`），采用统一边框、圆角与 Lucide 矢量图标，杜绝 Emoji 与风格割裂；
  - 点击平滑展开嵌入式抽屉卡片，包含场景需求多行输入框、在线模型双层伪下拉选择器及「智能生成」按钮；
  - 遵循现代极简中性灰设计风格（`.ai-gen-drawer`, `.ai-gen-textarea`, `.ai-gen-result-box`）。
...
- [x] 82.4 **端到端实机验证与质量闭环**：
  - 25 项 Rust 单元测试（`cargo test`）全绿通过；
  - Chrome 实机端到端模拟测试（按钮样式像素级对齐、伪下拉展开收起与双向绑定、需求输入 -> 模型请求 -> 字段渲染 -> 勾选应用 -> 模态弹窗弹出）全部正常。
