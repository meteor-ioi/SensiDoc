...
- [x] 82.2 **右侧面板操作按钮统合、标题重构与两段式折叠抽屉设计 (`web/index.html`, `web/style.css`)**：
  - 标题精简为「**规则列表 (X 项)**」，释放顶部视觉层级；
  - 将「常用标签库」由原本常驻占位的独立区域升级为折叠抽屉卡片（`#tagPoolDrawer`），默认优雅收起；
  - 在「新增规则」前新增「**标签库**」实体按钮，与「AI智能生成」、「新增规则」以及左侧「导入文件」全面统合为标准 `.btn.sm` 样式，像素级统一边框与圆角；
  - 「标签库」抽屉与「AI智能生成」抽屉具备丝滑展开/收起及互斥联动机制。
...
- [x] 82.4 **端到端实机验证与质量闭环**：
  - 25 项 Rust 单元测试（`cargo test`）全绿通过；
  - Chrome 实机端到端模拟测试（按钮样式像素级对齐、标题动态计数、标签库抽屉展开/收起/互斥/快速注入规则、伪下拉展开收起与双向绑定）全部正常。
