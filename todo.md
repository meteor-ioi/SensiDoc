...
- [x] 82.2 **右侧面板操作按钮统合、标题极简化与轻质抽屉面板重构 (`web/index.html`, `web/style.css`)**：
  - 标题彻底精简为纯粹的「**规则列表**」，移除末尾冗余的 `(X 项)` 统计内容；
  - 抽屉面板视觉重塑：背景由生硬暗沉的深灰转为系统统一纯白（`var(--surface)`），边框由深灰强描边转为细腻柔和的标准浅灰（`var(--border)`），搭配 8px 圆角与柔和微阴影，彻底解决颜色过深不协调问题；
  - 移除抽屉右上角冗余的关闭叉号按钮，统一通过点击操作按钮收放抽屉；
  - 将「常用标签库」升级为轻质折叠抽屉（`#tagPoolDrawer`），在「新增规则」前新增「**标签库**」实体按钮；
  - 将「**新增规则**」换回与上方「另存新模板」完全统一的蓝色文字链接样式（`.btn-text-link`），与实体功能按钮错落有致、主次分明。
...
- [x] 82.4 **端到端实机验证与质量闭环**：
  - 25 项 Rust 单元测试（`cargo test`）全绿通过；
  - Chrome 实机端到端模拟测试（抽屉柔和卡片质感、标题纯粹无尾缀、关闭按钮剔除、新增规则蓝色文字链接样式、标签库抽屉展开/收起/互斥/快速注入规则、伪下拉展开收起与双向绑定）全部正常。
