...
- [x] 82.2 **右侧面板操作按钮统合、标题极简化与轻质抽屉面板重构 (`web/index.html`, `web/style.css`)**：
  ...
  - 将「常用标签库」升级为轻质折叠抽屉（`#tagPoolDrawer`），在「新增规则」前新增「**标签库**」实体按钮，与「AI智能生成」、「新增规则」及「导入文件」保持 `.btn.sm` 像素级一致。
...
- [x] 82.4 **端到端实机验证与质量闭环**：
  - 25 项 Rust 单元测试（`cargo test`）全绿通过；
  - Chrome 实机端到端模拟测试（抽屉柔和卡片质感、标题纯粹无尾缀、关闭按钮剔除、标签库抽屉展开/收起/互斥/快速注入规则、伪下拉展开收起与双向绑定）全部正常。
